...
export const axiosDirectusInstance = axios.create({
  baseURL: import.meta.env.VITE_API_DIRECTUS_URL,
  headers: {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${import.meta.env.VITE_API_DIRECTUS_AUTHORIZATION}`,
  },
});

axiosDirectusInstance.interceptors.request.use(
  (config) => {
...
axiosDirectusInstance.interceptors.response.use(
  (response) => {
